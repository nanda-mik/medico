...
exports.addPrescription = async (req, res, next) => {
    ...
    try {
        ...
        if (presc) {
            ...
            sub.forEach(async (eachSub) => {
                var pushConfig = {
                    endpoint: eachSub.endpoint,
                    keys: {
                        auth: eachSub.keys.auth,
                        p256dh: eachSub.keys.p256dh
                    }
                };

                const payload = JSON.stringify({
                    title: 'New Notification',
                    body: 'New prescriptions added.',
                })

                const not = await webpush.sendNotification(pushConfig, payload);
                console.log(not);
            })
        } else {
